            ...
            //result.movieNm , result.rank , result.movieCd
               ...
               $.ajax({
                     ...
                     success: result2=>{
						...
						$("tbody").children(`:eq(${result.rank})`).append(`<th></th>`);
						$("tbody").children(`:eq(${result.rank})`).children(`:eq(${2})`).append(`<button class="btn btn-primary movieTitle${result.rank}">영화선정</button>`)
...
//반복문이 끝나고 영화가 끝나고
	...
	$(".buttonadd").click(function(){
		...
		var item={theaterName:theaterName,cinemaCode:cinemaCode,movieName:movieNm,movieDate:movieDate,movieTime:movieTime}
		 $.ajax({
